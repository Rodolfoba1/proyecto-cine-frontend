...
// Define correctamente los tipos para la página
type Props = {
  params: { id: string }
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  return {
    title: `Detalles de Reservación #${params.id}`
  }
}
...
export default async function ReservationDetailsPage({ params }: Props) {
  ...
  if (!reservation || String(reservation.userId) !== String(session.id)) {
  ...
  const qrData = JSON.stringify({
    ...
    seats: reservation.seats.map(
      (s) => `${String.fromCharCode(65 + s.row)}${s.column + 1}`
    ),
  })
                  ...
                  <div>
                    ...
                    <p>
                      ...
                      {reservation.seats
                        .map(
                          (s) =>
                            `${String.fromCharCode(65 + s.row)}${s.column + 1}`
                        )
                        .join(", ")}
                    </p>
                    <p>
                      <span className="font-semibold">Total pagado:</span> $
                      {reservation.seats.length * 50}.00
                    </p>
                    <p>
                      <span className="font-semibold">Reservado el:</span>{" "}
                      {format(parseISO(reservation.createdAt), "PPP", {
                        locale: es,
                      })}
                    </p>
                  </div>
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent className="p-6 flex flex-col items-center">
                ...
                <p className="text-sm text-slate-400 text-center mt-4">
                  Presenta este código QR en la entrada del cine para acceder a
                  tu función.
                </p>
              </CardContent>
            </Card>
